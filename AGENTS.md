...
8. **Native ownership is not Java reachability**: Open streams need owner leases, every active downcall needs short-lived operation leases, and `hs_alloc_scratch` output must be installed even on failure. Retain handles when close/free fails, release leases only after native consumption, and never let callback exceptions cross an FFM upcall boundary. See `docs/knowledge-base/panama-ffm/native-lifecycle-and-upcalls.md`.
9. **Windows ISA detection**: `PROCESSOR_IDENTIFIER` is not a feature list. Use HotSpot `UseAVX`/`UseSSE` for automatic tier selection and conservatively fall back to baseline when diagnostics are unavailable. See `docs/knowledge-base/panama-ffm/windows-isa-detection.md`.
